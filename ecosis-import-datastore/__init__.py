...
def test():
    ...
    client = MongoClient(config.get("app:main", "esis.mongo.url"))
    db = client[config.get("app:main", "esis.mongo.db")]

    collections = {
        "spectra" : db["workspace_spectra"],
        "resource" : db["workspace_resources"],
        "package" : db["workspace_packages"],
        "usda" : db[config.get("app:main", "esis.mongo.usda_collection")]
    }

    init(schema, collections, pgConn, config.get("app:main", "esis.search_url"))
